...
Где что синхронизировано
  ...
  - Чтение идет через FileChannel.read() с абсолютной позицией — это безопасно даже при конкурентной
  записи в тот же FileChannel                                                                            
  - Писатель держит UnifiedLog.lock, но читатель его не берет                                          
  ...
  Несколько request handler threads могут конкурировать за lock, но только один пройдет — остальные      
  ...
  достигается на уровне разных партиций, а пропускная способность — за счет батчинга (много сообщений за 
