...
BunnyMQ -- распределенный брокер сообщений с топиками, партициями и группами консьюмеров на базе Raft-репликации. Развертывается как кластер из нескольких узлов (минимум 3) в локальной сети или облачной инфраструктуре.
...
+ `pkg/client` -- Go-библиотека для встраивания в прикладной код продюсеров и консьюмеров.
...
#gost-table(num: "2.1", caption: "Минимальные требования к среде выполнения")[
  #table(
    ...
    [Оперативная память],   [512~МБ на узел (без учета данных журналов)],
    [Дисковое хранилище],   [SSD рекомендован; объем --- retention\_bytes × число партиций на узел],
...
BunnyMQ не требует внешних баз данных, объектных хранилищ или брокеров -- все данные хранятся локально на диске каждого узла. При развертывании в Docker контейнеризация опциональна.
...
#gost-table(num: "3.1", caption: "Основные параметры конфигурации")[
  #table(
    ...
    [`defaults.retention_bytes`],   [`-1`],    [-1 означает без ограничения по объему],
  )
]
...
Первый запуск (bootstrap) занимает несколько секунд, пока Raft-кластер проводит первичные выборы лидера, а готовность к приему запросов определяется по строке в логе:
...
Сначала процесс перестает принимать новые RPC и дожидается завершения текущих, после чего синхронизирует данные на диск и закрывает Raft-слой. Типичное время при нагруженном узле -- до 10~секунд.
...
#gost-table(num: "5.1", caption: "Типовые ответы и действия оператора")[
  #table(
    ...
    [`NOT_FOUND`],          [Топик или партиция не существует],   [Проверить имя топика и число партиций],
...
#gost-table(num: "5.2", caption: "Типовые сообщения в логах и действия")[
  #table(
    ...
    [`retention: deleted segment`],   [Сегмент удален по политике хранения; норма],
