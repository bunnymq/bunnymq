...
BunnyMQ -- распределённый брокер сообщений с топиками, партициями и группами консьюмеров на базе Raft-репликации. Система рассчитана на эксплуатацию в виде кластера из нескольких узлов (минимум 3) в локальной сети или облачной инфраструктуре.
...
Первый запуск (bootstrap) занимает несколько секунд, пока Raft-кластер проводит первичные выборы лидера, а готовность к приёму запросов определяется по строке в логе:
...
Процесс завершается поэтапно: сначала перестаёт принимать новые RPC, дожидается завершения текущих, синхронизирует Storage на диск, после чего закрывает dragonboat NodeHost. Типичное время при нагруженном узле -- до 10~секунд.
