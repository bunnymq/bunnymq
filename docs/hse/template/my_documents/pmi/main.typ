...
BunnyMQ -- брокер сообщений для распределенных backend-приложений. Хранит порядок сообщений внутри каждой партиции, реплицирует данные через Raft и масштабируется за счет партиционирования.
...
- `internal/coordinator/cluster`, `internal/coordinator/data`, `internal/coordinator/group` --- координаторы кластера, данных и групп консьюмеров;
...
+ корректность работы подсистемы хранения (раздел~4.1 ТЗ): сегментирование журнала, индексирование по смещению и времени, политика хранения;
...
+ корректность алгоритма Range Assignment для групп консьюмеров (раздел~4.1 ТЗ);
...
+ `Fetch` с `max_wait_ms > 0` --- чтение данных по смещению с long polling при отсутствии данных.
...
+ `GetOffsets(ByTimestamp)` --- получение первого смещения за заданное время.

=== Группы консьюмеров

+ `JoinGroup` --- регистрация участника в группе консьюмеров.
+ `Heartbeat` --- обновление записи об активности участника.
...
# Чтение с long polling (ожидание до 5 с)
./bunnymq-cli consume --topic testTopic --partition 0 \
