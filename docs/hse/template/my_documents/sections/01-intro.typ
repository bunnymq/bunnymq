...
+ `bunnymq` -- серверный процесс брокера, запускается на каждом узле кластера и обеспечивает прием и хранение сообщений, репликацию данных и экспорт метрик;
+ `bunnymq-cli` -- утилита командной строки для управления кластером, топиками и отладки: просмотр состояния кластера, управление топиками, публикация и чтение сообщений;
...
- параллельная обработка данных несколькими консьюмерами через группы с автоматическим перераспределением партиций.
