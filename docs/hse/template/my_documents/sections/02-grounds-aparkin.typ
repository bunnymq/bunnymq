...
Разработка серверной части 1 платформы CSTATI.events (events-service, payment-service, qr-service, инфраструктура) ведется на основании следующих документов:
...
+ *payment-service* --- микросервис обработки онлайн-платежей через интеграцию с платежным шлюзом ЮKassa;
...
Разработчик (М.М. Апаркин) отвечает за разработку, интеграцию и тестирование указанных выше трех микросервисов, а также конфигурацию инфраструктуры и DevOps-конвейера. Остальные компоненты (auth-manager, notifier, tg-bot, frontend, Nginx) разрабатываются другими членами команды и интегрируются в единую систему в соответствии с определенным в общем ТЗ взаимодействием через REST API, gRPC и Kafka.
